...
// must store it as int in order to use qsort 
   int open_today;
   int pre_close;
   int current_price;
   int max;
   ...
   int boll_daily_upper;
   int boll_daily_middle;
   int boll_daily_lower;
   ...
   int ddr;
   float liutongA;
   long long sort_liutongA;
   float totals;
   long long sort_totals;
   float value_liutongA;
   long long sort_value_liutongA;
   float value_totals;
   long long sort_value_totals;
...
#define PROCPATHLEN 128  // must hold /proc/2000222000/task/2000222000/cmdline

typedef struct PROCTAB 
{
   DIR*	data_root_fs;  // used within readproc.c
//   char deBug0[64];
   DIR*	taskdir;  // for threads
//   char deBug1[64];
   pid_t	taskdir_user;  // for threads used within readproc.c
   ...
   pid_t*	pids;	// pids of the procs used within readproc.c
   uid_t*	uids;	// uids of procs used within readproc.c

 ...
 */
const char *Head_nlstab[P_MAXPFLAGS];
const char *Desc_nlstab[P_MAXPFLAGS];
...
/* Translation Notes ------------------------------------------------
   ...
   . */
#ifdef BUILD_4_STOCK
/* Translation Hint: maximum 'StockID' = 7 */
#ifdef CHINESE
   Head_nlstab[S_StockID] = _("股票代码");
   Head_nlstab[S_StockName] = _("股票名称");
#else
   Head_nlstab[S_StockID] = _("StockID");
   Head_nlstab[S_StockName] = _("StockName");
#endif
   Desc_nlstab[S_StockID] = _("Stock ID");
   Desc_nlstab[S_StockName] = _("Stock Name");

   Head_nlstab[S_INDEX] = _("Index");
   Desc_nlstab[S_INDEX] = _("Index");

   Head_nlstab[S_OPEN] = _("Open T");
   Desc_nlstab[S_OPEN] = _("Open today price");
   
   Head_nlstab[S_CLOSE_YESTODAY] = _("Close Y");
   Desc_nlstab[S_CLOSE_YESTODAY] = _("Close yestoday price");

   Head_nlstab[S_MIN] = _("MIN");
   Desc_nlstab[S_MIN] = _("Min price");

   Head_nlstab[S_MAX] = _("MAX");
   Desc_nlstab[S_MAX] = _("MAX price");

   Head_nlstab[S_CURRENT] = _("Current");
   Desc_nlstab[S_CURRENT] = _("Current price");

   Head_nlstab[S_VOLUME] = _("Volume");
   Desc_nlstab[S_VOLUME] = _("Volume");

   Head_nlstab[S_RMB] = _("Amount");
   Desc_nlstab[S_RMB] = _("Amount(CNY)");

   Head_nlstab[S_PERCENT] = _("UP Down");
   Desc_nlstab[S_PERCENT] = _("UP Percent");

   Head_nlstab[S_TURNOVER_RATE] = _("TORate");
   Desc_nlstab[S_TURNOVER_RATE] = _("TurnOver Rate");

   Head_nlstab[S_LIUTONGA] = _("LiuTongA");
   Desc_nlstab[S_LIUTONGA] = _("LouTong gu");

   Head_nlstab[S_TOTALS] = _("Totals");
   Desc_nlstab[S_TOTALS] = _("Totals");

   Head_nlstab[S_VALUE_TOTALS] = _("Value Total");
   Desc_nlstab[S_VALUE_TOTALS] = _("Value Total");

   Head_nlstab[S_VALUE_LIUTONGA] = _("ValueLTA");
   Desc_nlstab[S_VALUE_LIUTONGA] = _("Value of LiuTongA");

   Head_nlstab[S_DAY] = _("Date");
   Desc_nlstab[S_DAY] = _("Date");

   Head_nlstab[S_TIME] = _("Time");
   Desc_nlstab[S_TIME] = _("Time");

   Head_nlstab[S_VRATIO] = _("VRatio");
   Desc_nlstab[S_VRATIO] = _("Volume Ratio");

   Head_nlstab[S_AVERAGE_PRICE] = _("Average");
   Desc_nlstab[S_AVERAGE_PRICE] = _("Average price");

   Head_nlstab[S_SWING] = _("Swing"); 
   Desc_nlstab[S_SWING] = _("Swing"); 

   Head_nlstab[S_OUTER] = _("Outer");
   Desc_nlstab[S_OUTER] = _("Outer");

   Head_nlstab[S_INNER] = _("Inner"); 
   Desc_nlstab[S_INNER] = _("Inner"); 

   Head_nlstab[S_PE_RATIO] = _("PE");
   Desc_nlstab[S_PE_RATIO] = _("PE Ratio");

   Head_nlstab[S_MA5] = _("MA5");
   Desc_nlstab[S_MA5] = _("MA5");

   Head_nlstab[S_MA10] = _("MA10");
   Desc_nlstab[S_MA10] = _("MA10");

   Head_nlstab[S_MA20] = _("MA20");
   Desc_nlstab[S_MA20] = _("MA20");

   Head_nlstab[S_MA30] = _("MA30");
   Desc_nlstab[S_MA30] = _("MA30");

   Head_nlstab[S_MA60] = _("MA60");
   Desc_nlstab[S_MA60] = _("MA60");

   Head_nlstab[S_DDR] = _("DDR");
   Desc_nlstab[S_DDR] = _("DDR");

   Head_nlstab[S_MNR] = _("MNR");
   Desc_nlstab[S_MNR] = _("Main Net Rate");

   Head_nlstab[S_SWEIGHT] = _("Predict");
   Desc_nlstab[S_SWEIGHT] = _("Price Predict");

   Head_nlstab[S_DOWNSHADOW] = _("DShadow");
   Desc_nlstab[S_DOWNSHADOW] = _("Down Shadow");
...
/* Translation Hint: Only the following words should be translated
   ...
   Norm_nlstab[USAGE_abbrev_txt] = _("\n"
        " -b : Bach mode operation\n"
        " -c : Command line/Program name toggle\n"
        " -d : Delay time interval\n"
        " -h : Help\n"
        " -H : Threads toggle\n"
        " -i : Show Idle [stock filtered]\n"
//        " cHiOSs\n"
        " -n : Iterations\n"
        " -o : field\n"
        " -O : showall fields\n"
        " -p : Monitor pid [,stockid...]\n"
   ...
   Norm_nlstab[BAD_stockid_txt] = _("Invalid stockid");
...
/* Translation Notes ------------------------------------------------
   ...
   Uniq_nlstab[KEYS_helpbas_fmt] = _(""
      ...
      "  u,U,o,O,= Filter by: '~1u~2'/'~1U~2' effective/any user; '~1o~2'/'~1O~2' other criteria; '=' cancel criteria\n"
...
{
//modify#ifdef VALIDATE_NLS
//modify   static const char *nls_err ="\t%s_nlstab[%d] == NULL\n";
//modify   int i;
//modify
//modify   setlocale (LC_ALL, "");
//modify   bindtextdomain(PACKAGE, LOCALEDIR);
//modify   textdomain(PACKAGE);
//modify
//modify   memset(Head_nlstab, 0, sizeof(Head_nlstab));
//modify   memset(Desc_nlstab, 0, sizeof(Desc_nlstab));
//modify   build_two_nlstabs();
//modify   for (i = 0; i < P_MAXPFLAGS; i++) {
//modify      if (!Head_nlstab[i]) {
//modify         fprintf(stderr, nls_err, "Head", i);
//modify         exit(1);
//modify      }
//modify      if (!Desc_nlstab[i]) {
//modify         fprintf(stderr, nls_err, "Desc", i);
//modify         exit(1);
//modify      }
//modify   }
//modify   memset(Norm_nlstab, 0, sizeof(Norm_nlstab));
//modify   build_norm_nlstab();
//modify   for (i = 0; i < norm_MAX; i++)
//modify      if (!Norm_nlstab[i]) {
//modify         fprintf(stderr, nls_err, "Norm", i);
//modify         exit(1);
//modify      }
//modify   memset(Uniq_nlstab, 0, sizeof(Uniq_nlstab));
//modify   build_uniq_nlstab();
//modify   for (i = 0; i < uniq_MAX; i++)
//modify      if (!Uniq_nlstab[i]) {
//modify         fprintf(stderr, nls_err, "Uniq", i);
//modify         exit(1);
//modify      }
//modify#else
   setlocale (LC_ALL, "");
...
#ifdef LOG2STDOUT
   fprintf(LogPtr, "PACKAGE_NAME = %s\n", PACKAGE_NAME);         //modify
   fprintf(LogPtr, "PACKAGE_VERSION = %s\n", PACKAGE_VERSION);   //modify
   fprintf(LogPtr, "LOCALEDIR = %s\n", LOCALEDIR);               //modify
   fprintf(LogPtr, "norm_MAX = %d\n", norm_MAX);
   fprintf(LogPtr, "uniq_MAX = %d\n", uniq_MAX);
   fprintf(LogPtr, "P_MAXPFLAGS = %d\n", P_MAXPFLAGS);
   fflush(LogPtr);
#endif
